...
all: checkdevice showbuild $(BUILD_DIR) $(FIRMW_DIR) $(FIRMW_DIR)/rom0.bin $(FIRMW_DIR)/rom1.bin htmlflash
...
flash:
	$(Q) $(ESPTOOL) -p $(ESPPORT) write_flash -fs $(ET_FS) \
		...
		0xff000 $(FIRMW_DIR)/webpages.espfs

webpages.espfs: html/ mkespfsimage/mkespfsimage
	cd html; find | ../mkespfsimage/mkespfsimage  > ../$(FIRMW_DIR)/webpages.espfs; cd ..

mkespfsimage/mkespfsimage: mkespfsimage/
	make -C mkespfsimage

htmlflash: webpages.espfs
	@if [ $$(stat -c '%s' $(FIRMW_DIR)/webpages.espfs) -gt $$(( 0x2E000 )) ]; then echo "webpages.espfs too big!"; false; fi
